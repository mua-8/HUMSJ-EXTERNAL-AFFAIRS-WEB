...
const Donate = () => {
  ...
  return (
    <Layout>
      {/* Hero Section */}
      <section className="relative py-20 bg-gradient-to-br from-[#1F8B86] to-[#25A7A1] overflow-hidden">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
                ...
                <Card variant="white" className="shadow-md">
                  <CardHeader>
                    <CardTitle className="flex items-center gap-2 text-[#1f2937]">
                      <Gift className="w-5 h-5 text-[#25A7A1]" />
                  ...
                  <CardContent className="space-y-6">
                    ...
                    <div className="grid grid-cols-3 gap-3">
                      {donationAmounts.map((amount) => (
                        <button
                          ...
                          className={`p-4 rounded-xl border-2 text-center transition-all ${selectedAmount === amount
                              ? "border-[#25A7A1] bg-[#E8F6F5] text-[#25A7A1]"
                              : "border-[#e5e7eb] hover:border-[#25A7A1]/50 text-[#374151]"
                            }`}
                        >
                    ...
                    {getFinalAmount() > 0 && (
                      <div className="bg-[#E8F6F5] rounded-xl p-4 text-center">
                        <p className="text-sm text-[#374151]">Your donation</p>
                        <p className="text-3xl font-bold text-[#25A7A1]">
              ...
              <div id="payment-methods" className="space-y-6">
                <Card variant="white" className="shadow-md">
                  <CardHeader>
                    <CardTitle className="flex items-center gap-2 text-[#1f2937]">
                      <CreditCard className="w-5 h-5 text-[#25A7A1]" />
                      Payment Methods
                    </CardTitle>
                    <CardDescription>
                  ...
                  <CardContent className="space-y-4">
                    {bankAccounts.map((account, index) => (
                      <div
                        key={index}
                        className="p-4 rounded-xl border border-[#e5e7eb] hover:border-[#25A7A1]/30 transition-colors"
                      >
                        <div className="flex items-start gap-3">
                          <div className="w-10 h-10 rounded-lg bg-[#E8F6F5] flex items-center justify-center flex-shrink-0">
                            <account.icon className="w-5 h-5 text-[#25A7A1]" />
                          </div>
                          <div className="flex-1">
                            <p className="font-medium text-[#1f2937]">{account.bank}</p>
                            <p className="text-sm text-[#6b7280]">{account.accountName}</p>
                            <div className="flex items-center gap-2 mt-2">
                              <code className="bg-[#f3f4f6] px-3 py-1 rounded text-[#25A7A1] font-mono text-sm">
                              ...
                              >
                                {copiedField === `account-${index}` ? (
                                  <Check size={16} className="text-[#25A7A1]" />
                                ) : (
                  ...
                  <CardContent>
                    <ol className="space-y-3 text-sm text-[#374151]">
                      <li className="flex gap-3">
                        <span className="w-6 h-6 rounded-full bg-[#25A7A1] text-white flex items-center justify-center text-xs flex-shrink-0">1</span>
                        <span>Select or enter your donation amount above</span>
                      </li>
                      <li className="flex gap-3">
                        <span className="w-6 h-6 rounded-full bg-[#25A7A1] text-white flex items-center justify-center text-xs flex-shrink-0">2</span>
                        <span>Copy the account number of your preferred payment method</span>
                      </li>
                      <li className="flex gap-3">
                        <span className="w-6 h-6 rounded-full bg-[#25A7A1] text-white flex items-center justify-center text-xs flex-shrink-0">3</span>
                        <span>Transfer the amount using your bank app or Telebirr</span>
                      </li>
                      <li className="flex gap-3">
                        <span className="w-6 h-6 rounded-full bg-[#25A7A1] text-white flex items-center justify-center text-xs flex-shrink-0">4</span>
                ...
                <div className="text-center p-4 bg-[#fef9c3] rounded-xl border border-[#d4af37]/30">
                  ...
                  <a
                    ...
                    className="inline-flex items-center gap-2 text-[#25A7A1] font-medium hover:underline"
                  >
                ...
                {[
                  ...
                  <Card key={index} variant="default" className="text-center">
                    <CardContent className="pt-6">
                      <p className="text-2xl font-bold text-[#25A7A1] mb-2">{item.amount}</p>
